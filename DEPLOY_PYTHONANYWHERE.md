...
- **Адрес сервера в лаунчере**: поля ввода в интерфейсе нет — адрес берётся
  автоматически в таком порядке:
  1. `%LOCALAPPDATA%\SteamCloneLauncher\config.json` → ключ `server_url`
     (создаётся сам после входа/регистрации);
  2. файл `server.txt` рядом с `SteamCloneLauncher.exe` (одна строка:
     `https://логин.pythonanywhere.com`) — если в конфиге адреса нет;
  3. переменная окружения `STEAM_CLONE_URL`;
  4. по умолчанию `http://127.0.0.1:8000` (локальный `py manage.py runserver`).
